...
>
> (2026.03.01 수정) Ubuntu 24.04 LTS가 2024년 4월 출시 이후 약 2년이 지나 안정성이 검증됐다. 신규 서버 구축 시 24.04 LTS (지원 종료 2029년 4월) 를 권장한다. 22.04도 2027년까지는 사용 가능하지만 신규 프로젝트는 24.04로 시작하자.
>
> - 안정성: 최신 버전은 예상치 못한 버그나 호환성 문제가 발생할 수 있음 (이 원칙 자체는 유효)
> - 커뮤니티 지원: 24.04는 이제 충분한 레퍼런스와 트러블슈팅 자료가 축적됨
> - 패키지 호환성: Python, Docker, PostgreSQL 등 주요 패키지의 검증된 버전 조합 사용 가능
> - 장기 지원: Ubuntu 24.04 LTS는 2029년 4월까지 공식 지원
...
> ~~- 정확도와 속도의 균형을 맞춘 방식~~
> ~~- 다른 인덱스 옵션: `hnsw` — 더 빠르지만 메모리 사용량 높음 (대규모 데이터에 유리)~~
>
> (2026.03.01 수정) pgvector 0.7 이후 hnsw가 사실상 표준으로 교체됐다.
>
> hnsw (Hierarchical Navigable Small World):
> - 그래프 기반 인덱스. 계층적 탐색으로 매우 빠른 ANN 검색
> - 데이터 삽입 시마다 인덱스가 유지되어 실시간 삽입에 강함
> - ivfflat 대비 검색 속도와 정확도 모두 우수
> - 단점: 메모리 사용량이 ivfflat보다 높음 (수백만 건 이상 시 주의)
>
> ivfflat은 개념 이해용으로만 참고하고, 실무에서는 hnsw를 사용하자.
>
> 언제 인덱스를 만드는가?
